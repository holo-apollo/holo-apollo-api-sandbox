...
export default class Subscription extends Component {
  ...
  validateError(values) {
    ...
    if (!values.email) {
      ...
      return {
        email: gettext("Oops... There's a mistake. Please type a valid email."),
      };
    }
    return { email: null };
  }
    ...
    return (
      <div
        className={cx('subscription-form', {
          hidden: this.state.submitSuccess || this.state.submitPending,
        })}
      >
        <h5>{gettext("Get notified when it's ready")}</h5>
        <Form
              ...
              <form onSubmit={formApi.submitForm}>
                <div className={'inputs'}>
                  <Text
                    ...
                    placeholder={'Email'}
                  />
                  <Button type={'submit'}>{gettext('Subscribe')}</Button>
                </div>
                <div className={'error'}>
    ...
    return (
      <div
        className={cx('unsubscribe', {
          hidden: this.state.unsubscribeSuccess || this.state.submitPending,
        })}
      >
