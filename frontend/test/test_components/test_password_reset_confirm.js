...
window.django_data.urls.passwordResetConfirmAPI =
...
describe('PasswordResetConfirm component', function() {
  ...
  it('should render form', function() {
    expect(this.wrapper.find('.password-reset-confirm-form').exists()).to.equal(
      true
    );
    expect(
      this.wrapper.find('.password-reset-confirm-form.hidden').exists()
    ).to.equal(false);
  });
...
describe('Password reset form submit', function() {
  ...
  it('should submit form successfully', function(done) {
    ...
    expect(
      this.fakePost.calledWith('rest-auth/password/reset/confirm/', values)
    ).to.equal(true);
    promise.then(() => {
  ...
  it('should handle known submit error', function(done) {
    const promise = Promise.reject({
      response: { data: { new_password1: 'Known error' } },
    });
    const values = {
      new_password1: 'P@ssw0rd',
      new_password2: 'P@ssw0rd',
    ...
    promise
      ...
      .catch(() => {
        expect(this.wrapper.state('submitPending')).to.equal(false);
        expect(this.wrapper.state('submitErrors').new_password1).to.equal(
          'Known error'
        );
        done();
      });
